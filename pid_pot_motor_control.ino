...
const int pidKI = 1; //integral value; default 5; maybe 0.08?
const int pidKD = 0; //derivative value; default 1
const int pidSampleTime = 500; //ms
const int pidSetPoint = 100; //this is probably an analog read value, or maybe RPM?
...
volatile int msPerCycle = 100; //ms
...
void setup()
{
  ...
  input = map(analogRead(chanA), 0, 1023, 0, 255);
...
void loop()
{
  ...
  //code for testing
  analogWrite(mosfet, map(analogRead(A0), 0, 1023, 0, 255));
